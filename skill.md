...
### 5.8 生成 kbase/workflow.md

开发工作流文档，从 L1 配置提取的开发命令 + 环境变量 + 部署配置生成。

**仅当项目有 package.json scripts（dev/build/test 等）或有 Dockerfile 时生成。**

```markdown
# 开发工作流

> 本文档沉淀 {项目名称} 项目的日常开发工作流。
...
| 步骤 | 命令 | 说明 |
|------|------|------|
| 安装依赖 | `{install command}` | {包管理器}, {lock 文件} |
| 启动开发服务器 | `{dev command}` | {端口}, {需要的环境变量} |
| 构建 | `{build command}` | {输出目录} |
| 类型检查 | `{typecheck command}` | {配置文件} |
| 测试 | `{test command}` | {测试框架} |
| 代码检查 | `{lint command}` | {linter} |

## 二、环境变量

| 变量 | 必须 | 默认值 | 说明 | 来源文件 |
|------|------|--------|------|---------|
| {VAR} | {是/否} | {default} | {说明} | {文件} |
...
- [ ] 实现程度的状态标注附有验证方式（命令或文件路径）

### workflow.md DoD
- [ ] 包含快速启动表（安装/开发/构建/测试命令）
- [ ] 包含环境变量表
- [ ] 内容来源于实际项目配置，非推断

### INDEX.md DoD (新增)
- [ ] 反向索引表已生成（至少 3 条文件→知识映射）
